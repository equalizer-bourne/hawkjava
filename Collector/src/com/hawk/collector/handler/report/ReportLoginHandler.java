...
public class ReportLoginHandler implements HttpHandler {
	/**
	 * 格式: game=%s&platform=%s&server=%s&puid=%s&device=%s&playerid=%d&playerlevel=%d&period=%d&time=%s
	 */
	@Override
	...
	public static void doReport(Map<String, String> params) throws Exception {
		if (params != null) {
			...
			// 更新失败即插入新记录
			sql = String.format("INSERT INTO login(game, platform, server, channel, puid, device, playerid, playerlevel, period, time, date) VALUES(%s);", value);
