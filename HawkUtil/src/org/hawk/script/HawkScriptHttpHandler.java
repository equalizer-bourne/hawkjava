		...
		try {
			...
			if (HawkScriptManager.getInstance().getScriptConfig().isWhiteIptables()) {
				...
				if (!HawkNetManager.getInstance().checkWhiteIptables(ip)) {
					HawkLog.logPrintln(String.format("script request closed by white iptables, ipaddr: %s", ip));
					HawkScriptManager.sendResponse(httpExchange, "script request closed by white iptables");
					return;
				}
			}
			...
			if (!HawkScriptManager.getInstance().checkUser(user)) {
				result += "script user is banned: " + user;
				...
				if (checkSysCommand(command)) {
					if (user != null && user.equals(HawkApp.getInstance().getAppCfg().getAdmin())) {
						...
						if ("suspend".equals(command)) {
							suspend = true;
							HawkScriptManager.sendResponse(httpExchange, "script handler suspend");
						} 
	
						// 恢复脚本服务
						if ("resume".equals(command)) {
							suspend = false;
							HawkScriptManager.sendResponse(httpExchange, "script handler resume");
						} 
						...
						if ("shell".equals(command)) {
							result = onShellCommand(HawkScriptManager.paramsToMap(params));
							if (result != null) {
								HawkScriptManager.sendResponse(httpExchange, result);
							}
						} 
						...
						if ("shutdown".equals(command)) {						
							HawkScriptManager.sendResponse(httpExchange, "script shutdown");
